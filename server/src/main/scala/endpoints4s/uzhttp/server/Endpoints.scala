...
trait EndpointsWithCustomErrors extends algebra.EndpointsWithCustomErrors with Requests with Responses {
  ...
  override def choiceRequestEntity[A, B](
    ...
    req =>
      requestEntityA(req).zip(requestEntityB(req)).map {
        case (Valid(v), _)        => Valid(Left[A, B](v))
        case (_, Valid(v))        => Valid(Right[A, B](v))
        case (invalidA, invalidB) => invalidA.zip(invalidB).asInstanceOf[Invalid]
      }
}
